...
function dselect(el, option = {}) {
  ...
  const searchInput = search ? `<input onkeydown="return event.key !== 'Enter'" onkeyup="dselectSearch(event, this, '${classElement}', '${classToggler}', ${creatable})" type="text" class="form-control rounded-0" placeholder="Search" autofocus>` : ''
  ...
  function createDom() {
    ...
    ` : ''
    const template = `
    <div class="dropdown ${classElement} ${additionalClass} rounded-0">
      <button class="rounded-0 ${classToggler} ${!el.multiple && clearable ? classTogglerClearable : ''}" data-dselect-text="${!el.multiple && selectedText(el.options)}" type="button" data-bs-toggle="dropdown" aria-expanded="false"${autoclose}>
